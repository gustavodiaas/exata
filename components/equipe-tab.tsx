...
export function MasterTab() {
  ...
  const carregarClientes = async () => {
    setIsLoading(true)
    try {
      // Busca todos os perfis para a visão master
  ...
  const toggleStatus = async (id: string, currentStatus: string) => {
    ...
    try {
      ...
      if (error) throw error
      toast({ title: "Comando executado", description: `O acesso foi alterado para ${novoStatus.toUpperCase()}.` })
      carregarClientes()
    } catch (error: any) {
  ...
  const handleCriarConvite = async () => {
    ...
    setIsCreating(true)
    try {
      // Pega a sua identidade para provar que é um Master operando
      ...
      const response = await fetch('/api/admin/invite', {
        ...
        body: JSON.stringify({ 
          ...
          empresaNome: novaEmpresa.trim(), 
          password: 'Exata@123',
          gerenteId: user?.id,
          nivel: 'admin' // Comando direto para criar um cliente
        })
      })
      ...
      toast({ title: "Fábrica Operacional", description: "Admin gerado com sucesso. Senha padrão: Exata@123" })
      setNovaEmpresa("")
      setNovoEmail("")
      setIsAdding(false)
      carregarClientes()
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Gestão absoluta da base de dados e clientes.</p>
        </div>
      ...
      <div className="bg-card border border-border rounded-2xl shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-border flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-foreground">Usuários no Sistema</h3>
          </div>
                ...
                {isLoading ? (
                  ...
                  clientes.map((cliente) => (
                    <tr key={cliente.id} className="hover:bg-muted/10 transition-colors">
                      <td className="px-6 py-4 font-bold text-foreground">{cliente.email}</td>
